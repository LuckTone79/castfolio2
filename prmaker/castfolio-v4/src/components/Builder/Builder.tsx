...
import { Save, Eye, Rocket, Layout, Type, Image as ImageIcon, Briefcase, Star, Phone, Sparkles, ChevronLeft, ChevronRight, X, Plus, Trash2, Smartphone, Monitor, AlignLeft, AlignCenter, AlignRight, Bold, Italic, Type as TypeIcon, Palette, MoveVertical, MoveHorizontal, ChevronUp, ChevronDown, MessageCircle, QrCode, Search } from 'lucide-react';
...
const Builder: React.FC<BuilderProps> = ({ talent, onBack }) => {
  ...
  useEffect(() => {
    const fetchPage = async () => {
      ...
      try {
        console.log('Builder: Fetching page for talent:', talent.id, 'Agent:', user.uid);
        
        // Query pages for this talent. We include agentId to satisfy security rules
        const q = query(
          collection(db, 'pages'), 
        ...
        console.error('Builder: Error in fetchPage:', error);
        const errorMessage = error?.message || 'Unknown error';
        setError(`Failed to load builder: ${errorMessage}`);
        
        // If it's a permission error, it might be because of missing index or rule issue
        if (errorMessage.includes('permission') || errorMessage.includes('insufficient')) {
          console.error('Builder: Permission denied. Check firestore.rules and query filters.');
        }
      }
    };
    ...
    <div className="flex h-screen bg-gray-50 overflow-hidden">
      ...
      <div className="w-80 bg-white border-r border-gray-200 flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-2">
            {[
              { id: 'theme', icon: Layout, label: t.theme },
              { id: 'hero', icon: Type, label: t.hero },
              { id: 'about', icon: Type, label: t.about },
              { id: 'career', icon: Briefcase, label: t.career },
              { id: 'portfolio', icon: ImageIcon, label: t.portfolio },
              { id: 'strengths', icon: Star, label: t.strengths },
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className="flex-1 overflow-hidden flex">
          {/* Editor Panel */}
          {!isPreview && (
            <div className="w-96 bg-white border-r border-gray-200 overflow-y-auto p-8">
                  ...
                  <motion.div key="theme" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} className="space-y-8">
                    ...
                    <div className="p-4 bg-indigo-50 rounded-2xl border border-indigo-100 space-y-4">
                      <h4 className="text-sm font-bold text-indigo-900 flex items-center gap-2">
                        <Layout size={16} />
                        <span>{t.sectionVisibility} & {t.sectionOrder}</span>
                      </h4>
                      <div className="space-y-2">
                        {(page.sectionOrder || ['hero', 'about', 'career', 'portfolio', 'strengths', 'contact']).map((section, idx) => (
                          <div key={section} className="flex items-center justify-between bg-white p-3 rounded-xl border border-indigo-100 group">
                            <div className="flex items-center gap-3">
                              <div className="flex flex-col gap-1">
                                ...
                                <button 
                                  ...
                                  disabled={idx === (page.sectionOrder?.length || 6) - 1}
                                  className="p-0.5 hover:bg-gray-100 rounded disabled:opacity-30"
                                  title={t.moveDown}
                                >
                                  <ChevronDown size={14} />
                              ...
                              <span className="text-sm font-medium text-gray-700">{(t as any)[section]}</span>
                            </div>
                            ...
                            <div className="flex bg-gray-100 rounded-lg p-1">
                              {[true, false].map((val) => (
                                <button
                                  key={String(val)}
                                  onClick={() => handleToggleSection(section, val)}
                                  className={`px-3 py-1 text-xs rounded-md transition-all ${
                                    (page.visibleSections?.[section as keyof typeof page.visibleSections] ?? true) === val
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-1 gap-3">
                        {LAYOUTS.map((l, idx) => (
                          <button
                            key={l.id}
                            onClick={() => setPage({ ...page, layout: l.id })}
