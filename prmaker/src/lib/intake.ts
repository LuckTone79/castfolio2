...
export function createEmptyIntakePayload(nameKo = "", nameEn = ""): IntakePayload {
  return {
    basic: {
      nameKo,
      nameEn,
...
export function payloadToPageContent(payload: IntakePayload): PageContent {
  ...
  const channels = [
    ...
    payload.contact.kakaoOpenChat ? { type: "kakao" as const, value: payload.contact.kakaoOpenChat, label: "카카오 오픈채팅" } : null,
    payload.contact.instagram ? { type: "instagram" as const, value: payload.contact.instagram, label: "인스타그램" } : null,
  ...
  return {
    ...
    profile: {
      ...
      profilePhotoUrl: payload.about.profilePhotoUrl,
      infoItems,
...
export function mergeIntakeContent(existingContent: PageContent, submittedContent: PageContent, mode: IntakeMergeMode): PageContent {
  if (mode === "overwrite") {
    return {
      ...existingContent,
      ...submittedContent,
  ...
  return {
    hero: {
      ...
      nameKo: firstFilled(existingContent.hero.nameKo || "", submittedContent.hero.nameKo || ""),
      nameEn: firstFilled(existingContent.hero.nameEn || "", submittedContent.hero.nameEn || ""),
      ...
      photoUrl: firstFilled(existingContent.hero.photoUrl || "", submittedContent.hero.photoUrl || ""),
    },
    profile: {
      ...
      profilePhotoUrl: firstFilled(existingContent.profile.profilePhotoUrl || "", submittedContent.profile.profilePhotoUrl || ""),
    ...
    career: { items: existingContent.career.items.length > 0 ? existingContent.career.items : submittedContent.career.items },
    portfolio: {
...
export function hasMeaningfulContent(content: PageContent) {
  return (
    Boolean(content.hero.nameKo) ||
    Boolean(content.hero.nameEn) ||
...
function firstFilled(existing: string, incoming: string) {
