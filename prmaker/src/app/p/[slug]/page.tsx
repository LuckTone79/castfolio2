...
export async function generateMetadata({ params }: Props) {
  ...
  const fallbackPosition = "방송인";

  return {
    title: `${talent.nameKo} | ${content?.hero?.position || fallbackPosition}`,
  ...
  return (
    <div>
      <div className="fixed top-4 right-4 z-50 flex gap-1 bg-white/90 backdrop-blur rounded-full shadow px-2 py-1">
        {[{ code: "ko", label: "KO" }, { code: "en", label: "EN" }, { code: "zh", label: "ZH" }].map((l) => (
          <a
            key={l.code}
          ...
          {content.contact.channels.slice(0, 3).map((ch, i) => {
            ...
            return (
              <a
                key={i}
                href={href}
                target={["email", "phone"].includes(ch.type) ? undefined : "_blank"}
                rel="noopener noreferrer"
                className="flex-1 flex flex-col items-center py-3 text-xs text-gray-600 hover:bg-gray-50"
              >
                <span className="text-[11px] font-semibold">{labels[ch.type] || "링크"}</span>
