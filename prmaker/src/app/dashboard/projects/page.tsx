  ...
  return (
    <div className="space-y-5 fade-in">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="font-bold text-xl" style={{ color: "var(--text-primary)" }}>제작 프로젝트</h1>
          <p className="text-xs mt-0.5" style={{ color: "var(--text-muted)" }}>
            총 {projects.length}건의 제작 흐름을 관리하고 있습니다.
          </p>
        </div>
        <Link href="/dashboard/projects/new" className="btn-primary flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm" style={{ background: "var(--accent)" }}>
          <PlusIcon /> 프로젝트 생성
        </Link>
      </div>
      ...
      {projects.length === 0 ? (
        <div className="rounded-2xl py-20 text-center" style={{ background: "var(--bg-surface)", border: "1px solid var(--border-default)" }}>
          ...
          <p className="font-semibold mb-1" style={{ color: "var(--text-primary)" }}>제작 프로젝트가 없습니다</p>
          <p className="text-sm mb-6" style={{ color: "var(--text-muted)" }}>
            방송인 고객을 선택하고 첫 PR 홈페이지 제작을 시작해보세요.
          </p>
          <Link href="/dashboard/projects/new" className="btn-primary inline-flex items-center gap-1.5 px-5 py-2.5 rounded-xl text-sm" style={{ background: "var(--accent)" }}>
            <PlusIcon /> 첫 프로젝트 만들기
          </Link>
        </div>
      ) : (
        <div className="rounded-2xl overflow-hidden" style={{ background: "var(--bg-surface)", border: "1px solid var(--border-default)" }}>
          <table className="w-full table-dark">
            <thead>
              <tr>
                {["프로젝트명", "방송인 고객", "진행 상태", "페이지 상태", "최종 수정일", "액션"].map((header) => (
              ...
              {projects.map((project) => {
                ...
                return (
                  <tr key={project.id}>
                    ...
                    <td>{project.talent.nameKo}</td>
                    <td>
                      <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${status.cls}`}>{status.label}</span>
                    </td>
                    <td>
                      {pageStatus ? (
                        <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${pageStatus.cls}`}>{pageStatus.label}</span>
                      ) : (
                    ...
                    <td>
                      <Link href={`/dashboard/projects/${project.id}`} className="text-xs font-medium" style={{ color: "#A78BFA" }}>
                        상세
                      </Link>
                    </td>
                  </tr>
