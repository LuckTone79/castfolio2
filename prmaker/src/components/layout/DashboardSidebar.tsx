...
const MENU_ITEMS = [
  { href: "/dashboard", label: "대시보드", Icon: IconHome, exact: true },
  { href: "/dashboard/talents", label: "방송인 고객", Icon: IconMic },
  { href: "/dashboard/intake", label: "자료 수집", Icon: IconInbox },
...
export const DashboardSidebar: React.FC<DashboardSidebarProps> = ({ user }) => {
  ...
  const portalLabel = user.userType ? "PARTNER PORTAL" : "ONBOARDING";

  return (
    <aside
      className="hidden md:flex flex-col h-full flex-shrink-0"
      style={{ width: 220, background: "var(--bg-surface)", borderRight: "1px solid var(--border-default)" }}
    >
      <div className="flex items-center gap-2.5 px-5" style={{ height: 56, borderBottom: "1px solid var(--border-subtle)" }}>
        <div>
          ...
          <p className="mt-1 text-[10px] font-bold tracking-[0.18em]" style={{ color: "var(--text-muted)" }}>
            {portalLabel}
          </p>
        </div>
      </div>
      ...
      <nav className="flex-1 px-2.5 py-3 space-y-0.5 overflow-y-auto">
        {MENU_ITEMS.map(({ href, label, Icon, exact }) => {
          ...
          return (
            ...
            >
              {isActive && <span className="absolute left-0 top-1/2 -translate-y-1/2 rounded-r-full" style={{ width: 3, height: 16, background: "#7C5CFC" }} />}
              <span className="flex-shrink-0 transition-opacity" style={{ opacity: isActive ? 1 : 0.55 }}>
                <Icon />
              </span>
              <span>{label}</span>
            </Link>
      ...
      <div className="px-2.5 py-3" style={{ borderTop: "1px solid var(--border-subtle)" }}>
        <div className="flex items-center gap-2.5 px-3 py-2 rounded-lg mb-1" style={{ background: "var(--bg-elevated)" }}>
          <div
            className="rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0"
            style={{ width: 28, height: 28, background: "linear-gradient(135deg, #4A36B8 0%, #7C5CFC 100%)", color: "#EEF2FF" }}
          >
            {initials}
          </div>
