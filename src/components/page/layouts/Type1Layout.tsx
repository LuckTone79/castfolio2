 ...
 *       ColorTheme prop → Type1Colors 매핑 → Provider로 모든 서브컴포넌트에 공급
 */
...
const DEFAULT_COLORS: Type1Colors = {
  bg:     "#FDF5F0",
  bg2:    "#F5EBE4",
  card:   "#FFFFFF",
  text:   "#2A1520",
...
function colorThemeToType1(ct: ColorTheme): Type1Colors {
  return {
    bg: ct.bg, bg2: ct.bgAlt, card: ct.card,
    text: ct.text, muted: ct.muted, muted2: ct.muted2,
    pink: ct.accent, pinkLt: ct.accentLight, pinkDk: ct.accentDark,
    plum: ct.dark, border: ct.border,
  };
}

const ColorCtx = createContext<Type1Colors>(DEFAULT_COLORS);
...
const Pill = ({ children }: { children: React.ReactNode }) => {
  ...
  return (
    <span style={{
      ...
      color: C.pinkDk,
      background: hexToRgba(C.pink, 0.10),
      border: `1px solid ${hexToRgba(C.pink, 0.25)}`,
...
const DarkPill = ({ children }: { children: React.ReactNode }) => {
  ...
  return (
    <span style={{
      ...
      letterSpacing: "0.2em", textTransform: "uppercase",
      color: C.pinkLt,
      background: hexToRgba(C.pinkLt, 0.15),
      border: `1px solid ${hexToRgba(C.pinkLt, 0.3)}`,
      padding: "4px 12px", borderRadius: 99, marginBottom: 12,
    }}>{children}</span>
  );
};
...
const HeroSection = ({
  ...
  const C = useC();
  const imgSrc = heroImageUrl || firstVideoUrl || `https://placehold.co/1280x720/${C.plum.replace("#","")}/E8A3B8?text=대표+영상`;
      ...
      <div style={{ maxWidth: 1040, margin: "0 auto", padding: "0 18px" }}>
        ...
        <motion.div
          ...
          style={{
            ...
            overflow: "hidden",
            background: C.plum,
            boxShadow: `0 16px 48px ${hexToRgba(C.plum, 0.2)}`,
          }}
        >
          <div style={{ position: "relative", width: "100%", paddingTop: "56.25%" }}>
            ...
            <div style={{
              ...
              background: `linear-gradient(150deg, ${hexToRgba(C.plum, 0.35)} 0%, ${hexToRgba(C.plum, 0.02)} 50%, ${hexToRgba(C.pink, 0.18)} 100%)`,
            }} />
            {/* 하단 메타 */}
            <div style={{
              position: "absolute", bottom: 0, left: 0, right: 0,
              padding: "32px 18px 18px",
              background: `linear-gradient(to top, ${hexToRgba(C.plum, 0.75)} 0%, transparent 100%)`,
            }}>
            ...
            <div style={{
              ...
              background: hexToRgba(C.plum, 0.6), backdropFilter: "blur(8px)",
        ...
        >
          ...
          {content.tagline && (
            <div style={{
              ...
              background: hexToRgba(C.pink, 0.1),
      ...
      <div style={{ maxWidth: 1040, margin: "0 auto", padding: "0 18px" }}>
        ...
        <div style={{
          ...
          {videos.length === 0 && [0, 1, 2, 3].map(i => (
            <div key={i} style={{
              flex: "0 0 160px", borderRadius: 14, overflow: "hidden",
              background: "rgba(255,255,255,0.06)",
              border: "1px solid rgba(255,255,255,0.1)",
            }}>
        ...
        {photos.length === 0 ? (
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
            {photos.map((id, i) => (
              ...
              >
                <img
                  src={photoUrls[id] || `https://placehold.co/600x800/${C.plum.replace("#","")}/E8A3B8?text=사진`}
                  alt={`Gallery ${i + 1}`}
...
export const Type1Layout: React.FC<Type1LayoutProps> = ({
  ...
  const colors = colorTheme ? colorThemeToType1(colorTheme) : DEFAULT_COLORS;
  const effectiveHeroImage = heroImageUrl || profileImageUrl;
  ...
  return (
    <ColorCtx.Provider value={colors}>
      <div style={{ fontFamily: "Pretendard, sans-serif", fontSize: 14, background: colors.bg, color: colors.text, overflowX: "hidden" }}>
        {watermark && (
          <div style={{
        ...
        <HeroSection
          ...
          firstVideoUrl={content.portfolio.videos[0]?.url}
        />
        ...
        {!isDisabled("contact") && (
          <ContactSection channels={content.contact.channels} />
        )}
